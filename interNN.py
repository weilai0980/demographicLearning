...
#   infer givn testing data    
    def inference(self, dx_test, cx_test, y_test, keep_prob):
        return self.sess.run([self.accuracy], feed_dict={self.dx:dx_test,\
                                                  self.cx:cx_test, self.y:y_test,\
                                                  self.keep_prob:keep_prob\
                                                   })
...
#       co-occurrence of categorical features
        for i in range(self.N_COOC):
            
            tmpvoca1 = self.N_DISC_VOCA[i] 
            
            for j in range(i+1,self.N_COOC):
...
#                   vector approximate of co-occurrence matrix 
                    w1 = tf.Variable(tf.random_normal([self.N_CLASS, tmpvoca1, 1],\
                                            stddev=math.sqrt(2.0/float( self.N_CLASS ))) )
...
#       dropout
#        x_concate = tf.nn.dropout(x_concate, self.keep_prob)
...
#   initialize loss and optimization operations for training
    def train_ini(self):
        self.loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(\
                                                    logits=self.logit, labels=self.y))         
        self.regu = self.L2*self.regularizer    
        
        self.cost = self.loss + self.regu
...
#   infer givn testing data    
    def inference(self, dx, cx, y, keep_prob):
        return self.sess.run([self.accuracy, self.loss, self.regu], feed_dict={self.dx:dx,\
                                                                               self.cx:cx, self.y:y,\
                                                                               self.keep_prob:keep_prob\
                                                   })
...
#   unit_test
    def test(self, dx_test, cx_test, y_test ):
        self.init = tf.global_variables_initializer()
        self.sess.run( self.init )
        return self.sess.run( [self.tmp1], \
                 feed_dict={self.dx:dx_test, self.cx:cx_test, self.y:y_test})
